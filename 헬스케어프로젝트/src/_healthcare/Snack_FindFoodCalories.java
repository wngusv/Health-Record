...
public class Snack_FindFoodCalories extends JFrame {
   ...
   public Snack_FindFoodCalories(String loginId) {
   	setIconImage(Toolkit.getDefaultToolkit().getImage(Snack_FindFoodCalories.class.getResource("/image/_ICON.png")));
   ...
   private void writeTodayEat() {
       String sqlSelectEatFood = "SELECT snack_meal, snack_kcal FROM snackdiet WHERE user_id = ? AND date = ? ";
       try (Connection conn = MySqlConnectionProvider.getConnection();
           ...
           try (ResultSet rs = pst.executeQuery();) {
               while (rs.next()) { // 먹은 음식 하나
                   ...
                   Double kcal = rs.getDouble("snack_kcal");
   ...
   private void findFoodUserEatOften() {
      ...
      String sql = "SELECT snack_meal FROM snackdiet\r\n" + "WHERE user_id = ? AND snack_meal != '단식'\r\n"
            + "GROUP BY snack_meal HAVING COUNT(*) >= 2";
      try (Connection conn = MySqlConnectionProvider.getConnection();
         ...
         try (ResultSet rs = pst.executeQuery()) {
            while (rs.next()) {
               String food = rs.getString("snack_meal");
   ...
   private void extracted() {
      ...
      lblNewLabel_6.setBounds(23, 397, 83, 51);
      lblNewLabel_6.setIcon(new ImageIcon(Snack_FindFoodCalories.class.getResource("/image/간식해.png")));
      getContentPane().add(lblNewLabel_6);

      JLabel lblNewLabel = new JLabel("간식 :");
      ...
      btnNewButton_1.addActionListener(new ActionListener() {
         public void actionPerformed(ActionEvent arg0) {
            String snack_meal = foodName;

            if (snack_meal != null && !snack_meal.isEmpty()) {
               String insertQeury = "INSERT INTO snackdiet (user_id, date, snack_meal, snack_kcal) VALUES (?,?,?,?)";
               try (Connection conn = MySqlConnectionProvider.getConnection();
                     PreparedStatement pst = conn.prepareStatement(insertQeury)) {
                  pst.setString(1, user_id);
                  pst.setDate(2, sqlDate);
                  pst.setString(3, snack_meal);
                  pst.setDouble(4, snack_kcal);
         ...
         public void valueChanged(ListSelectionEvent e) {
            if (!e.getValueIsAdjusting()) {
               ...
               if (selectedRow >= 0) {
                  ...
                  snack_kcal = (Double) resultTable.getValueAt(selectedRow, 2); // 더블형식의 아침 칼로리
