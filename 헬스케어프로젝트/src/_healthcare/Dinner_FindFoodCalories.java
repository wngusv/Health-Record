...
public class Dinner_FindFoodCalories extends JFrame {
   ...
   public Dinner_FindFoodCalories(String loginId) {
   	setIconImage(Toolkit.getDefaultToolkit().getImage(Dinner_FindFoodCalories.class.getResource("/image/_ICON.png")));
   ...
   private void writeTodayEat() {
       String sqlSelectEatFood = "SELECT dinner_meal, dinner_kcal FROM dinnerdiet WHERE user_id = ? AND date = ? ";
       try (Connection conn = MySqlConnectionProvider.getConnection();
           ...
           try (ResultSet rs = pst.executeQuery();) {
               while (rs.next()) { // 먹은 음식 하나
                   ...
                   Double kcal = rs.getDouble("dinner_kcal");
   ...
   private void findFoodUserEatOften() {
      ...
      String sql = "SELECT dinner_meal FROM dinnerdiet\r\n" + "WHERE user_id = ? AND dinner_meal != '단식'\r\n"
            + "GROUP BY dinner_meal HAVING COUNT(*) >= 2";
      try (Connection conn = MySqlConnectionProvider.getConnection();
         ...
         try (ResultSet rs = pst.executeQuery()) {
            while (rs.next()) {
               String food = rs.getString("dinner_meal");
   ...
   private void extracted() {
      ...
      lblNewLabel_6.setBounds(33, 397, 72, 51);
